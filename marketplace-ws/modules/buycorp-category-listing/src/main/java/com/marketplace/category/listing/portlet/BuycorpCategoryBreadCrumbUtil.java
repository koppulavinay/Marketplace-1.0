...
public class BuycorpCategoryBreadCrumbUtil {
	...
	public static List<BreadcrumbEntry> getAssetCategoriesBreadcrumbEntries(long categoryId,
		...
		vocabularyBreadcrumbEntry.setTitle(BuycorpCategoryListingPortletKeys.BREADCUMB_VOCABULARY);
		PortletURL portletURL = PortletURLBuilder.createRenderURL(renderResponse)
				.setMVCRenderCommandName(BuycorpCategoryListingPortletKeys.VIEW_URL).setNavigation(() -> {
					String navigation = ParamUtil.getString(httpServletRequest,
							BuycorpCategoryListingPortletKeys.NAVIGATION);
		...
		for (String categoryid : categoryIds) {
			...
			if (!categoryid.isEmpty()) {
				...
				portletURLStep.setParameter(BuycorpCategoryListingPortletKeys.PARENT_CATEGORYID,
						String.valueOf(categoryObj.getCategoryId()));
