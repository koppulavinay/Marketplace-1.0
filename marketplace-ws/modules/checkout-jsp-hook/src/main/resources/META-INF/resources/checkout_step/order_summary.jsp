...
<div class="mt-3">
	<liferay-ui:error key="oms-redirect-failure" message="Unable to process Order" />
	<liferay-ui:error key="bid-redirect-failure" message="Unable to process Bid" />
</div>

<div class="container containerMob minMamTableWidth checkoutPage">
	<div class="addProductTitle">
		<h1 class="titlePage mt-4">Checkout Summary</h1>
	</div>
		...
		<table class="table table-bordered" id="checkoutTable">
			<thead>
				<tr>
					...
					<th scope="col">Total Consignees</th>
				</tr>
			</thead>
			<tbody>
				<c:forEach items="${commerceOrderItems}" var="commerceOrderItem"
					...
					<tr class="nestedTable">
						...
						<td class="fontSize16">${commerceOrderItem.getTotalConsignees()}</td>
					</tr>
				</c:forEach>
			</tbody>
		</table>
...
<!-- Commented old code for checkout order summary page for future reference. -->
